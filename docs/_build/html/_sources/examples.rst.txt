...
.. code-block:: python
    ...
    resource = Resource(os.pathbasename(filename), contents)

    task = Task({"type": "sample", "kind": "raw"})

    task.add_resource("sample", resource)
    task.add_payload("tags", ["simple_producer"])
    task.add_payload("additional_info", ["This sample has been added through simple producer example"])
...
.. code-block:: python
        ...
        def process_sample(self):
            remoteResource = self.current_task.get_resource("sample")
            localResource = self.download_resource(remoteResource)
            # ...
