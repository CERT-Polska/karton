...
def test_logging(backend: KartonBackend, producer: Producer, service_backend: str):
    ...
    task = Task(headers={"instance": "first", "backend": service_backend, "type":"log-task", "message": log_message})

    logs_iterator = backend.consume_log(timeout=5, logger_filter=f"karton.test-{service_backend}-service-1")
