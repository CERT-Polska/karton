...
class SystemServiceConfig(Config):
    ...
    def __init__(self, path):
        ...
        self.buckets_config = dict(
            self.config.items("buckets")
            if self.config.has_section("buckets")
            else []
        )
        self.system_service_config = dict(
            self.config.items("system_service")
            if self.config.has_section("system_service")
            else []
        )
        self.splunk_config = dict(
            self.config.items("splunk")
            if self.config.has_section("splunk")
            else []
        )
        self.expiration_check_interval = int(
            self.system_service_config.get(
                "check_interval", SystemServiceConfig.DEFAULT_CHECK_INTERVAL
            )
        )
        self.default_bucket_expiration = int(
            self.system_service_config.get(
                "bucket_expiration",
    ...
    def bucket_expiration(self, bucket_name):
        return int(
            self.buckets_config.get(
                bucket_name, self.default_bucket_expiration
            )
        )
...
class SystemService(KartonBase):
    ...
    def __init__(self, config):
        ...
        splunk = splunklib.client.connect(
            host=self.config.splunk_config["host"],
    ...
    def process_expired(self):
        ...
        """
        if time.time() > (
            self.last_expiration_check + self.config.expiration_check_interval
        ):
                ...
                for bucket in buckets:
                    ...
                    for obj in objects:
                        try:
                            ...
                            o = self.minio.stat_object(
                                bucket_name=bucket.name,
                                object_name=obj.object_name,
                            )
                            seconds_since_now = time.mktime(
                                time.gmtime()
                            ) - time.mktime(o.last_modified)
                            if seconds_since_now > older_than:
                                resource = self._resource_object(
                                    bucket.name, obj.object_name
                                )
                                resource.remove(self.minio)
                                self.log.info(
                                    "Removed resource {}:{}".format(
                                        bucket.name, obj.object_name
                                    )
                                )
                        except minio.error.NoSuchKey:
                            self.log.warn(
                                "Couldn't remove object: %s" % obj.object_name
                            )
        ...
        if queue == "karton.tasks":
            ...
            for identity, raw_binds in self.rs.hgetall("karton.binds").items():
                ...
                if identity not in bound_identities:
                    self.log.info(
                        "Unbound identity detected: {}".format(identity)
                    )
                ...
                for bind in binds:
                    if all(
                        headers.get(bind_key) == bind_value
                        for bind_key, bind_value in bind.items()
                    ):
                        self.log.info(
                            "Task matched binds for {}".format(identity)
                        )
